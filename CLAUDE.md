...
| `up --seamless --party [--party-password X]` | account 2 keeps its white sign down and account 1 summons it by itself (`DS2PartyGuest`/`DS2PartyAccept`/`DS2PartyPassword`); a session forms and re-forms with no input, and the server matches party signs only with the same password. `DS2_Party.req` takes `pausa`/`retoma` |
